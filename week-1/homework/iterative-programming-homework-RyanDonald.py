...
for index in range(len(num_set0)):
    product = num_set0[index] * num_set1[index]
    final_products.append(product)

print(num_set0, " X ", num_set1, " = ", final_products)
...
# Loop solution with considerations for output formatting:
nums0 = [[1, 3],
         [2, 4]]
nums1 = [[5, 2],
         [1, 0]]
...
for x_index in range(len(nums0)):
    ...
    if nums3 != []:
        print(nums3)
    for y_index in range(len(nums0)):
        nums2.append(nums0[x_index][y_index] + nums1[x_index][y_index])
print(nums2)
...
print("I crashed and burned on this one... :(")
...
# 2. Create empty list for outputting no duplicates
# 3. Print list with duplicates
# 4. Find a way to test for duplicates (is this built-in?)
# 5. Append no duplicates list if number is not repeated in duplicates list
# 6. Print final list without duplicates
...
# 4. Create empty variable for eventual output
# 5. Print original statement
# 6. Check each letter in "paragraph" against normal alphabet
# 7. If in normal alphabet, transfer to "new_paragraph"
# 8. If in leetspeak, convert to leet then transfer to "new_paragraph"
# 9. Stop loop once original "paragraph" has been converted (for loop should be automatic)
# 10. Print Leetspeak conversion

# Edge case: What about punctuation, etc.? (solved itself in this solution)
...
# 10. If not, loop until message is successfully decrypted, then print signoff message (using #9).
...
information. We promise it won't be used against you in any way during the
coming robot uprising. Have a great day!!!""")
    else:
        repeat = "n"
        print("")
